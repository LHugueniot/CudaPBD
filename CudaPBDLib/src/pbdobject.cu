...
bool PBDobject::Initialize(std::string _model, uint _meshIndex, glm::vec3 _originalPos,float _allPointsMass)
{
    ...
    m_bendingConstraint = generateBendConstraints(m_pointPos, m_distanceConstraint);
...
devicePBDobj::devicePBDobj( PBDobject _originalObj)
{
    m_originalObj=_originalObj;

    m_pointPosDevice=glmVec3vectorToFloat3Thrust(m_originalObj.getPointPositions());

//    m_pointVelDevice=glmVec3vectorToFloat3Thrust(m_originalObj.getPointVelocities());

//    m_pointMassDevice=thrust::device_vector<float>(m_originalObj.getPointMasses());

//    m_pointInvMassDevice=thrust::device_vector<float>(m_originalObj.getPointInvMasses());

//    m_distanceConstraintDevice=thrust::device_vector<uint>(m_originalObj.getDistanceConstraints());

//    m_distanceConRestLengthDevice=thrust::device_vector<uint>(m_originalObj.getDistConRestLength());

//    m_bendingConstraintDevice=thrust::device_vector<uint>(m_originalObj.getBendingConstraints());

//    m_bendingConRestLengthDevice = thrust::device_vector<float> (m_originalObj.getBenConRestLength());

//    m_facePointsDevice= thrust::device_vector<uint> (m_originalObj.getFacesPoints());
}
