...
uint solver::addPBDobject(PBDobject *_PBDobjectPtr)
{
    m_PBDObjects.push_back(_PBDobjectPtr);
    return m_PBDObjects.size();
}

bool solver::rmPBDobject(PBDobject * _PBDobjectPtr)
{
...
bool solver::setPBDobject(uint index, PBDobject * _PBDobjectPtr)
{
    if(index<m_PBDObjects.size())
    {
...
std::vector<PBDobject *> solver::getObjects() const
{
    return m_PBDObjects;
}

//bool __host__ solver::InitGPUData()
//{
//    thrust::host_vector<PBDobject> hostPBDObj;
//    for(auto pbdobject: m_PBDObjects)
//    {

//    }
//    thrust::host_vector<LuHu::PBDobject>
//            thrust::device_vector<float3> a;
//}

void solver::RunSolver(float dt, uint iterations)
{
    float inv_dt = 1/dt;
    for(PBDobject * pObject :m_PBDObjects)
    {
