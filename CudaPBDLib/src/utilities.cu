...
const aiScene* getModel(std::string _model)
{
    ...
    std::string finalPath = std::string(getcwd(NULL, 0)) + std::string("/../../../models/") +_model;//std::string(buf).c_str()
...
thrust::host_vector<float3> glmVec3vectorToFloat3Thrust(std::vector<glm::vec3> stdVec)
{
    thrust::host_vector<float3> thrustVec(stdVec.size());
    for(uint it=0 ; it< stdVec.size(); it++)
    {
        thrustVec[it]=make_float3(stdVec[it].x, stdVec[it].y, stdVec[it].z);
    }
    return thrustVec;
}
